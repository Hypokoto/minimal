...
if [[ "$PKG_MGR" == "pacman" ]]; then
    ...
    echo ""
    read -t 10 -p "Would you like to configure Chaotic-AUR and Athena OS repositories? (Recommended) [Y/n] " prompt_repos || true
    echo ""
    if [[ "${prompt_repos:-y}" =~ ^[Yy]$ ]]; then
        log "Configuring Chaotic-AUR and Athena OS repositories..."
        # Chaotic AUR Setup
        ...
        if ! grep -q "\\[athena\\]" /etc/pacman.conf; then
            echo -e "\n[athena]\nSigLevel = Optional TrustAll\nServer = https://athena-os.github.io/\$repo/\$arch" | sudo tee -a /etc/pacman.conf
        fi
        sudo pacman -Sy
    else
        log "Skipping extra repositories configuration."
    fi
